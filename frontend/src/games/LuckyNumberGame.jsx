...
export default function LuckyNumberGame() {
  const apiFn = useCallback((bet, extra) => gameAPI.luckynumber(bet, extra), [])
  const { bet, setBet, result, spinning, play, chips, user } = useGame(apiFn)

  return (
    <GameLayout title="幸運數字" emoji="🎱">
      <div style={{ padding:'0 16px' }}>
        ...
        <div style={{ textAlign:'center', padding:'30px 0' }}>
          ...
          <div style={{ color:'#fff', fontSize:20, fontWeight:'bold', marginBottom:8 }}>幸運數字</div>
          {result && (

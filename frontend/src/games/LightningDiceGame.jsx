...
export default function LightningDiceGame() {
  const apiFn = useCallback((bet, extra) => gameAPI.lightningdice(bet, extra), [])
  const { bet, setBet, result, spinning, play, chips, user } = useGame(apiFn)

  return (
    <GameLayout title="閃電骰子" emoji="⚡">
      <div style={{ padding:'0 16px' }}>
        ...
        <div style={{ textAlign:'center', padding:'30px 0' }}>
          ...
          <div style={{ color:'#fff', fontSize:20, fontWeight:'bold', marginBottom:8 }}>閃電骰子</div>
          {result && (
